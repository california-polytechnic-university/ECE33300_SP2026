...
module block #(parameter xloc = 120,
   ...
   assign draw_block = (hcount <= xloc + xsize_div_2) &
                       (hcount >= xloc - xsize_div_2) &
                       (vcount <= yloc + ysize_div_2) &
                       (vcount >= yloc - ysize_div_2) ? ~broken : 1'b0;
      ...
      if (rst) begin
         ...
         if (unbreak) begin
            ...
            if (vcount >= yloc - (ysize_div_2 + 1) &&
                vcount <= yloc + (ysize_div_2 + 1)) begin
            ...
            if (hcount >= xloc - (xsize_div_2 + 1) &&
                hcount <= xloc + (xsize_div_2 + 1)) begin
               if (vcount == yloc + (ysize_div_2 + 1))
   ...
   always @(posedge clk or posedge rst) begin
      if (rst) begin
         ...
         if (unbreak) begin
            ...
            if (blk_lft | blk_dn | blk_up | blk_rgt)
               broken <= 1'b1;
         end
      end
   end
